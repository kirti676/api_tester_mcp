...
def _is_graphql_sdl(content: str) -> bool:
    ...
    return any(
        re.search(pattern, content, re.IGNORECASE) for pattern in graphql_patterns
    )
...
class ProgressTracker:
    ...
    def start(self):
        ...
        if self.enable_detailed_logging:
            ...
            logger.info(
                f"⏱️  Started at: {time.strftime('%H:%M:%S', time.localtime(self.start_time))}"
            )
    ...
    def update(self, step_name: str = None, force_log: bool = False):
        ...
        if milestone_hit:
            ...
            force_log
            or self.enable_detailed_logging
            and (self.current_step % max(1, self.total_steps // 10) == 0)
        ):
    ...
    def update_with_substep(self, step_name: str, substep: int, total_substeps: int):
        ...
        substep_percentage = (
            (substep / total_substeps) * 100 if total_substeps > 0 else 100
        )
        full_step_name = (
            f"{step_name} ({substep}/{total_substeps} - {substep_percentage:.1f}%)"
        )
    ...
    def finish(self):
        ...
        if self.start_time:
            ...
            avg_time_per_step = (
                duration / self.total_steps if self.total_steps > 0 else 0
            )
            ...
            if len(self.step_history) > 1:
                ...
                if len(recent_steps) > 1:
                    recent_duration = (
                        recent_steps[-1]["timestamp"] - recent_steps[0]["timestamp"]
                    )
                    recent_throughput = (
                        len(recent_steps) / recent_duration
                        if recent_duration > 0
                        else 0
                    )
                    logger.info(
                        f"   • Recent throughput: {recent_throughput:.2f} steps/sec"
                    )


def merge_env_vars(
    base_vars: Dict[str, str], new_vars: Dict[str, str]
) -> Dict[str, str]:
