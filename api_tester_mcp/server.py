...
ingested_file_directory: Optional[str] = (
    None  # Directory of the ingested API specification file
)
...
def get_workspace_dir() -> str:
    ...
    try:
        ...
        for _ in range(3):  # Check up to 3 levels up
            if any(
                os.path.exists(os.path.join(potential_workspace, indicator))
                for indicator in [
                    ".git",
                    ".vscode",
                    "package.json",
    ...
    fallback = os.getcwd()
    logger.warning(
        f"Using fallback workspace directory (may not be writable): {fallback}"
    )
    return fallback
...
def get_or_create_project_dir() -> str:
    ...
    if os.access(workspace_dir, os.W_OK):
        ...
        logger.warning(
            f"Workspace directory not writable, using anyway: {workspace_dir}"
        )
        return workspace_dir
...
def ensure_workspace_output_dir(subdir: str) -> str:
    ...
    try:
        ...
        logger.warning(
            f"Failed to create {target_dir}, falling back to workspace subdirectory: {str(e)}"
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to create fallback directory, using workspace directly: {str(e2)}"
            )
            return workspace_dir
...
class IngestSpecParams(BaseModel):
    ...
    preferred_framework: Optional[str] = (
        "requests"  # pytest, requests, playwright, jest, cypress, supertest
    )
...
class GenerateScenariosParams(BaseModel):
    include_negative_tests: bool = (
        True  # Generate failure scenarios (invalid data, unauthorized access)
    )
...
class RunApiTestsParams(BaseModel):
    test_case_ids: Optional[List[str]] = (
        None  # ["test_case_1", "test_case_2"] or None for all
    )
...
async def ingest_spec(params: IngestSpecParams) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Ingesting specification - file_path: {params.file_path}, "
        ...
        try:
            ...
            logger.info(
                f"Set workspace directory to ingested file location: {ingested_file_directory}"
            )
        ...
        if detected_type:
            if params.spec_type and detected_type != params.spec_type.lower():
                logger.warning(
                    f"Detected spec type '{detected_type}' differs from provided '{params.spec_type}', using detected type"
                )
        ...
        if params.preferred_language:
            try:
                ...
                logger.warning(
                    f"Invalid language '{params.preferred_language}', using default: python"
                )
        ...
        if params.preferred_framework:
            try:
                ...
                logger.warning(
                    f"Invalid framework '{params.preferred_framework}', using default: requests"
                )
        ...
        if required_vars:
            env_message.append(
                f"⚠️  {len(required_vars)} required environment variable(s) detected:"
            )
            for var_name, var_info in required_vars.items():
                if "detected_value" in var_info:
                    env_message.append(
                        f"   • {var_name}: {var_info['description']} (Suggested: {var_info['detected_value']})"
                    )
                else:
                    env_message.append(f"   • {var_name}: {var_info['description']}")
            env_message.append(
                "💡 Use set_env_vars() for configuration with automatic validation and guidance."
            )
        else:
            env_message.append(
        ...
        env_message.append("")
        env_message.append(
            f"📁 Workspace directory set to API specification file location:"
        )
        env_message.append(f"   {ingested_file_directory}")
        env_message.append(
            "   Generated files (scenarios, test cases, reports) will be saved here."
        )
...
async def set_env_vars(params: SetEnvVarsParams) -> Dict[str, Any]:
    ...
    try:
            ...
            suggested_variables[var_name] = {
                **var_info,
                ...
                "current_value": (
                    ...
                    if is_set
                    and (
                        "auth" in var_name.lower()
                        or "secret" in var_name.lower()
                        or "password" in var_name.lower()
                    )
                    else current_vars.get(var_name, None)
                ),
                "priority": "required",
            }
        ...
        for key, value in optional_fields.items():
            ...
            ):  # Only add if provided and not empty
        ...
        for var_name, var_info in suggested_variables.items():
            ...
            if var_name not in variables_to_set and "detected_value" in var_info:
                ...
                logger.info(
                    f"Auto-populated {var_name} with detected value from specification"
                )
            ...
            if var_name in suggested_variables:
                ...
                if var_info.get("priority") == "required" and not var_value.strip():
                    validation_errors.append(
                        f"Required variable '{var_name}' cannot be empty"
                    )
                ...
                validation_warnings.append(
                    f"Setting custom variable '{var_name}' not detected in API specification"
                )
        ...
        current_session.env_vars = merge_env_vars(
            current_session.env_vars, variables_to_set
        )
        ...
        for var_name, var_info in suggested_variables.items():
            if (
                var_info.get("priority") == "required"
                and var_name not in current_session.env_vars
            ):
        ...
        if len(missing_required) == 0:
            ...
            result["message"] = (
                f"⚠️ Configuration updated, but {len(missing_required)} required variable(s) still missing: {', '.join(missing_required)}"
            )
...
async def generate_scenarios(params: GenerateScenariosParams) -> Dict[str, Any]:
    ...
    try:
        ...
        endpoints = parser.parse(
            json.dumps(current_session.spec_content), current_session.spec_type
        )
        ...
        for endpoint in endpoints:
            ...
            progress.update(
                f"Generated scenarios for {endpoint.method} {endpoint.path}"
            )
        ...
        scenarios_file = os.path.join(
            scenarios_dir, f"scenarios_{current_session.id}.json"
        )
        ...
        return {
            ...
            "file_path": (
                scenarios_file if scenarios_file_created else "Failed to create"
            ),
            "message": (
                f"Scenarios saved to workspace: {scenarios_file}"
                if scenarios_file_created
                else f"Failed to create scenarios file"
            ),
        }
    ...
    try:
        ...
        if params.scenario_ids:
            scenarios_to_process = [
                scenario
                for scenario in current_session.scenarios
        ...
        generator = TestCaseGenerator(
            base_url, current_session.env_vars, language, framework
        )
        ...
        test_cases_file = os.path.join(
            test_cases_dir, f"test_cases_{current_session.id}.json"
        )
        ...
        try:
            ...
            logger.info(
                f"Successfully saved test cases metadata to workspace: {test_cases_file}"
            )
        except Exception as e:
        ...
        try:
            ...
            test_code = generator.code_generator.generate_test_code(
                test_cases, session_info
            )
        ...
        return {
            ...
            "file_path": (
                test_cases_file if test_cases_file_created else "Failed to create"
            ),
            ...
            + (
                f" | Code files: {', '.join([os.path.basename(f) for f in code_files_created])}"
                if code_files_created
                else ""
            )
            + (
                f" | Code file errors: {len(code_files_failed)}"
                if code_files_failed
                else ""
            ),
        }
...
async def run_api_tests(params: RunApiTestsParams) -> Dict[str, Any]:
    ...
    try:
        ...
        if params.test_case_ids:
            test_cases_to_run = [
                test_case
                for test_case in current_session.test_cases
        ...
        html_report = report_generator.generate_api_test_report(
            test_results, current_session
        )
        reports_dir = ensure_workspace_output_dir("reports")
        report_file = os.path.join(
            reports_dir, f"api_test_report_{current_session.id}.html"
        )
...
async def run_load_tests(params: RunLoadTestsParams) -> Dict[str, Any]:
    ...
    try:
        ...
        executor = LoadTestExecutor(
            duration=params.duration, users=params.users, ramp_up=params.ramp_up
        )

        load_test_results = await executor.run_load_test(test_cases_to_run)

        if "error" in load_test_results:
            return {"success": False, "error": load_test_results["error"]}

        # Generate HTML report
        html_report = report_generator.generate_load_test_report(
            load_test_results, current_session
        )
        reports_dir = ensure_workspace_output_dir("reports")
        report_file = os.path.join(
            reports_dir, f"load_test_report_{current_session.id}.html"
        )
...
async def generate_project_files(params: GenerateProjectParams) -> Dict[str, Any]:
    ...
    try:
        ...
        include_examples = (
            params.include_examples if params.include_examples is not None else True
        )
        ...
        for filename in expected_test_filenames:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    logger.warning(
                        f"Failed to read existing test file {file_path}: {str(e)}"
                    )
        ...
        if not existing_test_files:
            ...
            try:
                ...
                test_code = code_generator.generate_test_code(
                    current_session.test_cases, session_info
                )
        ...
        setup_instructions = _generate_setup_instructions(
            language_enum, framework_enum, project_name
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to create project directory {final_project_dir}: {str(e)}"
            )
        ...
        if existing_test_files:
            ...
            for filename, content in existing_test_files.items():
                ...
                if language_enum == TestLanguage.PYTHON:
                    if framework_enum == TestFramework.PYTEST:
                        project_test_path = os.path.join(
                            final_project_dir, "tests", filename
                        )
                    else:
                    ...
                    if framework_enum == TestFramework.CYPRESS:
                        project_test_path = os.path.join(
                            final_project_dir, "cypress", "e2e", filename
                        )
                    else:
                        project_test_path = os.path.join(
                            final_project_dir, "tests", filename
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to copy test file {project_test_path}: {str(e)}"
                    )
        else:
            ...
            try:
                ...
                for filename in expected_test_filenames:
                    ...
                    try:
                        ...
                        existing_test_files[filename] = (
                            test_code  # Add to existing for return data
                        )
                        logger.info(
                            f"Created fresh test file in project: {project_test_path}"
                        )
                    except Exception as e:
                        project_files_failed.append(
                            f"fresh test file {filename}: {str(e)}"
                        )
                        logger.error(
                            f"Failed to create fresh test file {project_test_path}: {str(e)}"
                        )
        ...
        return {
            ...
            "file_status": {
                "workspace_test_files_found": [
                    os.path.basename(f) for f in reused_files
                ],
                ...
                "total_project_files": len(project_files_created)
                + len(test_files_copied)
                + len(test_files_created_fresh),
            },
            "file_contents": {**package_files, **example_files, **existing_test_files},
            "setup_instructions": setup_instructions,
            "saved_files": saved_files,
            "message": f"Project created in {final_project_dir}. "
            + (
                f"Copied {len(test_files_copied)} test files from workspace. "
                if test_files_copied
                else ""
            )
            + (
                f"Created {len(test_files_created_fresh)} fresh test files. "
                if test_files_created_fresh
                else ""
            )
...
def _generate_setup_instructions(
    language: TestLanguage, framework: TestFramework, project_name: str
) -> List[str]:
    ...
    if language == TestLanguage.PYTHON:
        ...
        if framework == TestFramework.PYTEST:
            ...
            instructions.extend(
                ["4. Run tests:", "   ```bash", "   python test_api.py", "   ```"]
            )
...
async def get_workspace_info() -> Dict[str, Any]:
    ...
    try:
        ...
        if ingested_file_directory:
            workspace_info["message"] = (
                f"Workspace directory set from ingested API specification file location: {current_workspace}"
            )
        else:
            workspace_info["message"] = (
                f"Using default workspace directory: {current_workspace}"
            )
        ...
        return {"success": False, "error": f"Failed to get workspace info: {str(e)}"}
    ...
    try:
                ...
                for root, dirs, files in os.walk(output_base):
                    for file in files:
                        relative_path = os.path.relpath(
                            os.path.join(root, file), output_base
                        )
        ...
        return {
            ...
            "current_working_directory": (
                os.getcwd() if hasattr(os, "getcwd") else "unknown"
            ),
        }
...
async def get_report(report_id: str) -> Resource:
    ...
    try:
        report_file = os.path.join(
            ensure_workspace_output_dir("reports"), f"{report_id}.html"
        )
...
def main():
    ...
    if asgi_app is None:
        ...
        logger.info(
            "No ASGI accessor found; using mcp.run() with streamable-http transport"
        )
